...
if __name__ == "__main__":
    ...
    def compswap(env, verbose=False):
        ...
        if env.p1_pos == env.p2_pos and env.p2_pos < env.length-1:
            env._ptr_2_right()
    ...
    def rshift(env, verbose=False):
        ...
        init_state = env.get_state()
        if env.p1_pos < env.length-1:
            env._ptr_1_right()
        if env.p2_pos < env.length-1:
            env._ptr_2_right()
        state = env.get_state()
    ...
    def reset(env, verbose=False):
        ...
        init_state = env.get_state()
        while env.p1_pos > 0 or env.p2_pos > 0:
            lshift(env)
        state = env.get_state()
    ...
    def bubble(env, verbose=False):
        ...
        init_state = env.get_state()
        while env.p1_pos < env.length-1 or env.p2_pos < env.length-1:
            compswap(env)
            rshift(env)
        state = env.get_state()
    ...
    # Unit tests
    env = ListEnv(length=10, encoding_dim=conf.encoding_dim)
