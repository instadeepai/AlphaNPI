...
if __name__ == "__main__":
    ...
    def compswap(env, verbose=False):
        env.start_task(env.programs_library['COMPSWAP']['index'])
        assert env._compswap_precondition(), 'precondition not verified'
        init_state = env.get_state()
        if env.p1_pos == env.p2_pos and env.p2_pos < env.end_pos:
            env._ptr_2_right()
        idx_left = min(env.p1_pos, env.p2_pos)
        idx_right = max(env.p1_pos, env.p2_pos)
        if env.scratchpad_ints[idx_left] > env.scratchpad_ints[idx_right]:
            env._swap()
        state = env.get_state()
        assert env._compswap_postcondition(init_state, state), 'postcondition not verified'
        if verbose:
            print('program COMPSWAP')
            print('init_state: {}, end_state: {}, reward: {}'.format(env.get_state_str(init_state),
                                                                     env.get_state_str(state), env.get_reward()))
            print('')
        env.end_task()

    def rshift(env, verbose=False):
        env.start_task(env.programs_library['RSHIFT']['index'])
        assert env._rshift_precondition(), 'precondition not verified'
        init_state = env.get_state()
        if env.p1_pos < env.end_pos:
            env._ptr_1_right()
        if env.p2_pos < env.end_pos:
            env._ptr_2_right()
        state = env.get_state()
        assert env._rshift_postcondition(init_state, state), 'postcondition not verified'
        if verbose:
            print('program RSHIFT')
            print('init_state: {}, end_state: {}, reward: {}'.format(env.get_state_str(init_state),
                                                                     env.get_state_str(state), env.get_reward()))
            print('')
        env.end_task()

    def lshift(env, verbose=False):
        env.start_task(env.programs_library['LSHIFT']['index'])
        assert env._lshift_precondition(), 'precondition not verified'
        init_state = env.get_state()
        if env.p1_pos > env.start_pos:
            env._ptr_1_left()
        if env.p2_pos > env.start_pos:
            env._ptr_2_left()
        state = env.get_state()
        assert env._lshift_postcondition(init_state, state), 'postcondition not verified'
        if verbose:
            print('program LSHIFT')
            print('init_state: {}, end_state: {}, reward: {}'.format(env.get_state_str(init_state),
                                                                     env.get_state_str(state), env.get_reward()))
            print('')
        env.end_task()

    def reset(env, verbose=False):
        env.start_task(env.programs_library['RESET']['index'])
        assert env._reset_precondition(), 'precondition not verified'
        init_state = env.get_state()
        if env.p1_pos > env.start_pos or env.p2_pos > env.start_pos:
            lshift(env)
            reset(env)
        state = env.get_state()
        assert env._reset_postcondition(init_state, state), 'postcondition not verified'
        if verbose:
            print('program RESET')
            print('init_state: {}, end_state: {}, reward: {}'.format(env.get_state_str(init_state),
                                                                     env.get_state_str(state), env.get_reward()))
    ...
    def bubble(env, verbose=False):
        env.start_task(env.programs_library['BUBBLE']['index'])
        assert env._bubble_precondition(), 'precondition not verified'
        init_state = env.get_state()
        if env.start_pos < env.end_pos:
            compswap(env)
            rshift(env)
            bubble(env, False)
        state = env.get_state()
        assert env._bubble_postcondition(init_state, state), 'postcondition not verified'
        if verbose:
            print('program BUBBLE')
            print('init_state: {}, end_state: {}, reward: {}'.format(env.get_state_str(init_state),
                                                                     env.get_state_str(state), env.get_reward()))
            print('')
        env.end_task()

    def bubblesort(env, verbose=False):
        env.start_task(env.programs_library['BUBBLESORT']['index'])
        assert env._bubblesort_precondition(), 'precondition not verified'
        init_state = env.get_state()
        if not env._is_sorted():
            bubble(env)
            reset(env)
            bubblesort(env)
        state = env.get_state()
        assert env._bubblesort_postcondition(init_state, state), 'postcondition not verified'
        if verbose:
            print('program BUBBLESORT')
            print('init_state: {}, end_state: {}, reward: {}'.format(env.get_state_str(init_state),
                                                                     env.get_state_str(state), env.get_reward()))
            print('')
        env.end_task()
    ...
    # Unit tests
    env = RecursiveListEnv(length=10, encoding_dim=conf.encoding_dim)
    compswap(env, verbose=True)
    lshift(env, verbose=True)
    rshift(env, verbose=True)
    reset(env, verbose=True)
    bubble(env, verbose=True)
    bubblesort(env, verbose=True)
